...
/**
 * Função que encontra o menor título em ordem alfabética.
 *
 * @param arr Arranjo de Shows.
 * @param inicio Posição inicial.
 * @param n Tamanho arranjo.
 *
 * @return O índice do show de menor título em ordem alfabética.
 */
int indiceMenorShow(Show **arr, int inicio, int n)
{
    ...
    {
        if (compararShowsPorTitulo(arr[i], arr[minIndex]) < 0)
        {
            minIndex = i;
        }
    }
...
/**
 * Algoritmo de ordenação por seleção recursivo para Shows
 *
 * @param arr Arranjo de Shows
 * @param inicio Posição inicial
 * @param n Tamanho do arranjo de Shows
 */
void selectionSortRec(Show **arr, int inicio, int n)
{
    ...
    // Índice do menor
    int minIndex = indiceMenorShow(arr, inicio, n);

    // Troca
    if (minIndex != inicio)
    {
        swapShows(&arr[inicio], &arr[minIndex]);
    }
...
/**
 * Pega o maior valor de releaseYear
 *
 * @param array Arranjo de ponteiros para Shows
 * @param n Tamanho do array
 */
int getMaxReleaseYear(Show **array, int n)
{
    int max = array[0]->realeseYear;

    for (int i = 1; i < n; i++)
    {
        if (array[i]->realeseYear > max)
        {
            max = array[i]->realeseYear;
        }
    }
    return max;
}
 ...
 * @param array Arranjo de ponteiros para Shows
 * @param n Tamanho do array
 * @param exp Expoente (1, 10, 100, ...)
 */
void radcountingSortReleaseYear(Show **array, int n, int exp)
{
    // Definir dados
    Show *output[n];
    int count[10] = {0};

    // Contar ocorrências dos dígitos no releaseYear
    for (int i = 0; i < n; i++)
    {
        int digit = (array[i]->realeseYear / exp) % 10;
        count[digit]++;
    }

    // Transformar count[i] em posição final
    for (int i = 1; i < 10; i++)
    {
        count[i] += count[i - 1];
    }
...
/**
 * Algoritmo de ordenação RadixSort para Shows
 *
 * @param array Arranjo de ponteiros para Shows
 * @param n Tamanho do array
 */
void RadixSort(Show **array, int n)
{
    ...
    {
        radcountingSortReleaseYear(array, n, exp);
    }
}
...
int main()
{
    ...
    // Arquivo
    FILE *logFile = fopen("860144_bolha.txt", "w");
