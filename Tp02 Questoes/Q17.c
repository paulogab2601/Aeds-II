...
char **StrToArrayStringToken(char *str)
{
    ...
    // Definir dados para alocação -> A quantia de dados é o token + 1 = "Teste, 123, wasd" : 2 Virgulas e 3 dados
...
/**
 * Função de comparação para a ordenação, com base no diretor e no título.
 * Comparando os shows pela `director` e, em caso de empate, pelo `title`.
 */
...
/**
 * Função que ajusta o heap para garantir a propriedade de max-heap.
 * O show de índice i pode violar a propriedade do heap, e o heapify garante que o heap seja reconstruído.
 */
void heapify(Show **arr, int n, int i)
{
    ...
    // Se o maior não for o próprio índice, troque e faça heapify recursivamente
    if (largest != i)
    {
        Show *temp = arr[i];
        arr[i] = arr[largest];
...
/**
 * Função que constrói a heap (max-heap) a partir de um arranjo.
 */
void buildHeap(Show **arr, int n)
{
    // Construir a heap a partir do meio do arranjo
...
/**
 * Função para realizar o HeapSort (ordenação) dos Shows.
 */
void heapSort(Show **arr, int n)
{
    ...
    for (int i = n - 1; i > 0; i--)
    {
        // Troca o maior elemento (raiz) com o último elemento do heap
        Show *temp = arr[0];
        arr[0] = arr[i];
        arr[i] = temp;

        // Chama o heapify na raiz para garantir a propriedade do heap
        heapify(arr, i, 0);
    }
}
...
int main()
{
    ...
    // Arquivo
    FILE *logFile = fopen("860144_selecaoRecursiva.txt", "w");
