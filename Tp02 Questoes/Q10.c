...
/**
 * Algoritmo de ordenação por QuickSort para Shows
 * Baseado no esquema de particionamento de Hoare
 *
 * @param arr Arranjo de Shows
 * @param esq Posição inicial
 * @param dir Posição final
 */
void QuickSortShows(Show **arr, int esq, int dir)
{
    // Definir dados
    int i = esq, j = dir;
    ...
    while (i <= j)
    {
        ...
        // Encontra elemento maior que o pivô na esquerda
        while (compararShowsporData(arr[i], pivo) < 0)
        {
            comparacoes++;
            i++;
        }

        // Encontra elemento menor que o pivô na direita
        while (compararShowsporData(arr[j], pivo) > 0)
        {
            comparacoes++;
            j--;
        }
...
int main()
{
    ...
    QuickSortShows(ArrayShow, 0, j - 1);
