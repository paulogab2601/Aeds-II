 ...
 * @param cor Índice de cor atual
 * @param h Incremento utilizado
 */
void InsercaoPorCor(Show **array, int n, int cor, int h)
{
    for (int i = h + cor; i < n; i += h)
    {
        ...
        while (j >= 0 && (strcasecmp(array[j]->type, tmp->type) > 0 ||
                          (strcasecmp(array[j]->type, tmp->type) == 0 && strcasecmp(array[j]->title, tmp->title) > 0)))
        {
            array[j + h] = array[j];
            j -= h;
        }
...
int main()
{
    ...
    // Arquivo
    FILE *logFile = fopen("860144_shellsort.txt", "w");
